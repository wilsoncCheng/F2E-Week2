...
// 選取自行車都市
const bikeCity = document.querySelector('#bikeCity');
let targetcity="";
  ...
  axios({
    ...
    .then((response) => {
      ...
      CityData.forEach((item) => {
        str1 += `<option value="${item.CityName}">${item.CityName}</option>`
      })
      bikeCity.innerHTML = str1;

      bikeCity.addEventListener('change', (e) => {
...
function getRoutesData() {
  axios({
    method: 'get',
    url: 'https://ptx.transportdata.tw/MOTC/v2/Cycling/Shape/${bikeCity}',
    headers: GetAuthorizationHeader()
  })
    .then((response) => {
      ...
      routeData.forEach((item) => {
        str += `<option value="${item.AuthorityName}">${item.AuthorityName}</option>`
      })
